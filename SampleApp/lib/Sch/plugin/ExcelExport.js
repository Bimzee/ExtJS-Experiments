...
Ext.define("Sch.plugin.ExcelExport", {
    ...
    resourceGridHeader: 'Rig fields',
    resourcePrefix: 'Rig - ',
    eventPrefix: 'Job - ',
    eventGridHeader: 'Job fields',
    ...
    showFieldSelectionWindow: function() {
        if (!this.window) {
            var data = [];
            var hiddenFields = ['Id', 'TieIn', 'PermittedStatus', 'developmentType', 'APDNumber', 'TieInEndDate', 'TieInStartDate', 'titleClear',
                'locationReady', 'platRecieved', 'skidTime', 'Draggable', 'Resizable', 'CDC', 'ResourceId', 'ROI', 'TieInDuration', 'TieIn', 'childId',
                'parentId', 'Cls', 'Id', 'PadName', 'NPT', 'NptId', 'NptParentWellId', 'NptReason', 'NptDuration', 'NptStartDate', 'NptEndDate', 'NptComments',
                'NptLocked']
...
//
//                                resourceFields=['Name'];
//                                eventFields=['WellName','JobCategory','developmentType','StartDate','EndDate','AFE#','AFEStatus','AFEEstimate','WI','Duration','mobDays','demobDays','jobStatus','APDNumber',
//                                    'ActualBeginDate','ActualMobDate','ActualDemobDate','ActualEndDate','ActualDuration'];
//                            if (Ext.isIE) {
//                                me.ieToExcel(resourceFields, eventFields);
//                            } else {
//                                var xml = me.getExcelXml(resourceFields, eventFields);
//                                window.location = 'data:application/vnd.ms-excel;base64,' + xml;
//                            }
//                               Ext.getCmp('confirmExport').close();
//                         return "";
//                    },
//                    onComplete: function() {
//                        Ext.getCmp('confirmExport').close();
...
//                    downloadImage: 'resources/images/yes.png',
//                    width: 62,
//                    height: 22,
//                    transparent: true,
//                    append: false
//                });
//                Downloadify.create('cancelExport', {
//                    filename: function() {
//                        return "";
//                    },
//                    data: function() {
//                        Ext.getCmp('confirmExport').close();
//                        // var data = Ext.ux.exporter.Exporter.exportStore(store);
//                        return "";
//                    },
//                    onComplete: function() {
//                        Ext.getCmp('confirmExport').close();
...
//                    },
//                    swf: 'resources/flash/downloadify.swf',
//                    downloadImage: 'resources/images/no.png',
//                    width: 62,
//                    height: 22,
//                    transparent: true,
//                    append: false
                ...
                buttons: [
                    {
                        ...
                        handler: function() {
                            ...
                            if (!selector.isValid() || (selectedFields.length === 1 && !selectedFields[0]))
                                return;
    ...
    createWorksheet: function(resourceFields, eventFields) {
        ...
        for (var i = 0; i < nbrFields; i++) {
            var field = i < resourceFields.length ? this.grid.store.model.prototype.fields.get(resourceFields[i]) :
                    ...
                    header = header ? header.text : (field.text ? field.text : field.name);
        ...
        for (var i = 0, l = eventItems.length; i < l; i++) {
            ...
            for (var j = 0; j < nbrFields; j++) {
                ...
                if (cellType[j] == 'DateTime') {
                    ...
                    data += Ext.Date.format(new Date(v), 'm-d-Y');
                } else {
                    data += v;
                }
        ...
        for (var i = 0, l = eventItems.length; i < l; i++) {
            ...
            for (var j = 0; j < nbrFields; j++) {
                ...
                if (v instanceof Date) {
                    sheet.cells(i + 2, j + 1).value = Ext.Date.format(new Date(v), 'm-d-Y');
                } else {
