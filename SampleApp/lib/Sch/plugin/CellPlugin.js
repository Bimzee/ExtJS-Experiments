...
Ext.define('Sch.plugin.CellPlugin', {
    ...
    init  : function (scheduler) {
        ...
        me.lockedView.getSelectionModel().enableKeyNav = false;
        ...
        if (me.view.bufferedRenderer) {
            ...
            me.mon(me.view, 'rangefetched', me.onBufferedUpdate, me);
        }
        ...
        me.mon(me.view.resourceStore, {
            ...
            bulkremove          : me.onResourceRemove,
            scope               : me
        });
...
//        delete me.eventIndexInCell;
    ...
    onViewScroll        : function () {
        ...
        if (me.containerEl) {
            var node        = Ext.fly(me.view.getNodeByRecord(me.resource));
            if (node) {
    ...
    onResourceRemove    : function (store, record, index, isMove) {
        var me = this;
    ...
    addSelection        : function () {
        var me          = this;
        var prevFocus   = me.lockedView.getFocusEl();
    ...
    showEditorInCell    : function (params, e) {
        ...
        if (params.eventRecord) {
            me.alignEditorWithRecord(params.eventRecord);
        } else {
    ...
    alignEditorWithRecord   : function (record) {
        var me = this;
        
        // some enhancements to make box visible
        // TODO: use z-index or wrap box around event body
        var box = Ext.fly(me.view.getEventNodeByRecord(record)).getBox();
        box.y--;
        box.x--;
        me.alignEditor(box);
    },
    ...
    findPreviousIndex   : function (e) {
        ...
        for (var i = index - 1; i >= 0; i--) {
            // collapsed row is regular record but it has few additional properties like 'rows'
            // if row is collapsed, then we traverse further
            if (!me.getResource(i).hasOwnProperty('rows')) {
                return i;
            }
        }
    ...
    findNextIndex       : function (e) {
        ...
        for (var i = index + 1; i < me.rowsCount; i++) {
            // collapsed row is regular record but it has few additional properties like 'rows'
            // if row is collapsed, then we traverse further
            if (!me.getResource(i).hasOwnProperty('rows')) {
                return i;
    ...
    getCellEvents   : function (context) {
        ...
        events.sortBy(function (a, b) {
            return Ext.fly(me.view.getEventNodeByRecord(a)).getY() < Ext.fly(me.view.getEventNodeByRecord(b)).getY() ? -1 : 1;
        });
    ...
    expandResourceRow   : function (height) {
        ...
        if (record) {
            var eventBox = Ext.fly(me.view.getEventNodeByRecord(record)).getBox();
    ...
    beginEdit   : function () {
        ...
        me.prevFocusEl  = me.lockedView.getFocusEl();
        me.editing      = true; 
        ...
        if (event) {
            ...
            me.editor.recordNode = me.view.getEventNodeByRecord(event);
