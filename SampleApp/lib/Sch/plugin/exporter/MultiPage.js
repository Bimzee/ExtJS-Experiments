...
Ext.define('Sch.plugin.exporter.MultiPage', {
    ...
    onRowsCollected : function (lockedRows, normalRows) {
        ...
        // - build page frame for each page column,
        // - build pages by filling grids w/ rows
        // - finish exporting by launching `onPagesExtracted`
        me.buildPageFrames(function () { me.buildPages( me.onPagesExtracted, me, lockedRows, normalRows ); });
    },
    ...
    buildPageFrames : function (callback, scope) {
        var me                  = this,
                ...
                normalGridTable = me.normalGrid.body.down(me.tableSelector),
                totalWidth = lockedGrid.el.dom.clientWidth + normalGridTable.el.dom.clientWidth;
        ...
        var columnPagesNum      = me.getTotalWidth() > 0 ? Math.ceil(me.getTotalWidth() / me.paperWidth) : Math.ceil(totalWidth / me.paperWidth),
            pageFrames          = me.pageFrames = [];

        me.iterateAsync(function (next, colIndex, offset) {
            // ob build completion we call provided function
