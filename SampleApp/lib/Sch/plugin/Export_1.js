...
Ext.define('Sch.plugin.Export', {
    ...
    constructor: function(config) {
        ...
        if (config.exportDialogConfig) {
            Ext.Object.each(me.defaultConfig, function(k, v, o) {
    ...
    createExporter: function(className, config) {
        ...
        if (me.tpl)
            exportConfig.tpl = me.tpl;

        if (Ext.isObject(className)) {
            return Ext.create(Ext.apply(exportConfig, className));
        } else {
            return Ext.create(className, Ext.apply(exportConfig, config));
        }
    },
    /**
     * Adds an exporter.
    ...
    showExportDialog: function() {
        ...
        me.win = Ext.create(me.exportDialogClassName, {
            ...
            exportDialogConfig: Ext.apply({
                startDate: me.defaultConfig.startDate, //me.scheduler.getStart(),
                endDate: me.defaultConfig.endDate, //me.scheduler.getEnd(),
     ...
     *
     * @param {Object} [conf] Config options for exporting. If not provided, {@link #defaultConfig} is used. Possible parameters are :
    ...
    onExportSuccess: function(result) {
        var me = this,
                ...
                centerDate = me.exporter.restoreSettings.centerDate,
                win = me.getWin(),
...
//        if (me.exporter.config.exporterId === 'singlepage') {
//            timeAxis.setTimeSpan(scheduler.startDate, scheduler.endDate);
//        }

        setTimeout(function() {
            me.fireEvent('hidedialogwindow', result);
            scheduler.scrollToDateCentered(centerDate);
    ...
    onExportFailure: function(message, result) {
        ...
        Ext.ComponentQuery.query('button[itemId=exportCancel]')[0].setDisabled(false);
        Ext.ComponentQuery.query('button[itemId=exportButton]')[0].setDisabled(false);
...
//        }, win ? win.hideTime : 3000);
    ...
    doRequest: function(exportedPages, config) {
        ...
        if (!me.test && !me.debug) {
            if (window.ByteSize.count(Ext.JSON.encode(exportedPages)) <= 5000000) {
                if (me.printServer) {
                ...
                Ext.ComponentQuery.query('button[itemId=exportCancel]')[0].setDisabled(false);
                Ext.ComponentQuery.query('button[itemId=exportButton]')[0].setDisabled(false);
//                RigScheduler.getApplication().getUtilController().displayMessageBox("You cannot upload more than 5MB", 'ERROR', Ext.MessageBox.OK, Ext.MessageBox.ERROR);
                me.onExportFailure("You cannot upload more than 5 MB");
//                return false;
            }
