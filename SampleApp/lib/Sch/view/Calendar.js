...
Ext.define("Sch.view.Calendar", {
    ...
    */
    ...
    onEventUpdate: function (store, model) {
        ...
        // TODO: maybe we want to check if we really need to update column
    ...
    renderSingle : function (event) {
        ...
        var existing    = view.getEventNodesByRecord(event).elements;
        Ext.each(existing, function (el) {
            Ext.fly(el).destroy();
        });
    ...
    repaintEventsForColumn  : function (column, index) {
        ...
        if (events.length > 0) {
            for (i = 0, l = events.length; i < l; i++) {
                event   = events[i];
                node    = view.getEventNodesByRecord(event).elements[0];
        ...
        var trEl    = view.el.down('tr:nth-child(1)');
        ...
        for (i = 0; i < data.length; i++) {
            ...
            var fly = trEl.down('td:nth-child(' + (index + 1) + ')').select('[id^=' + event.id + '-]');
    ...
    */
    getTimeSpanRegion: function (startDate, endDate) {
        ...
        var pair2 = this.translateToScheduleCoordinate([endColumn.getRegion().right, 0]);
