...
Ext.define("Sch.view.Horizontal", {
    ...
    getEventRenderData: function (event) {
        ...
        var eventStart  = event.getStartDate(),
            ...
            data        = {},
            mobDays     = event.data.MobDays,
            deMobDays   = event.data.DeMobDays,
            mobStartX   = view.getXFromDate(Sch.util.Date.max(Sch.util.Date.add(eventStart, Sch.util.Date.DAY, -(mobDays)), viewStart)),
            mobEndtX    = view.getXFromDate(Sch.util.Date.max(eventStart, viewStart)),
            deMobStartX = view.getXFromDate(Sch.util.Date.max(eventEnd, viewStart)),
            deMobEndtX  = view.getXFromDate(Sch.util.Date.max(Sch.util.Date.add(eventEnd, Sch.util.Date.DAY, deMobDays), viewStart)),
            skidTimeStartX = view.getXFromDate(Sch.util.Date.max(Sch.util.Date.add(eventStart, Sch.util.Date.DAY, -(event.data.skidTime)), viewStart)),
            skidTimeEndX = view.getXFromDate(Sch.util.Date.max(eventStart, viewStart));

        data.data = event;
        data.eventprefix = view.eventPrefix;
        if (this.view.rtl) {
            data.right = M.min(startX, endX);
            data.mobRight = M.min(mobStartX, mobEndtX);
            data.deMobRight = M.min(deMobStartX, deMobEndtX);
            data.skidRight = M.min(skidTimeStartX, skidTimeEndX);
        } else {
            data.left = M.min(startX, endX);
            data.mobleft = M.min(mobStartX, mobEndtX);
            data.deMobleft = M.min(deMobStartX, deMobEndtX)- view.eventBorderWidth;
            data.skidleft = M.min(skidTimeStartX, skidTimeEndX);
        }

        data.width = M.max(1, M.abs(endX - startX)) - view.eventBorderWidth;
        data.mobWidth = M.max(1, M.abs(mobEndtX - mobStartX));// - view.eventBorderWidth;

        data.deMobWidth = M.max(1, M.abs(deMobEndtX - deMobStartX)) - view.eventBorderWidth;
        if (view.managedEventSizing) {
            ...
            if (data.data.get('TieIn')) {
                data.height = RigScheduler.config.Constants.getTieInJobHeight() - (2 * view.barMargin) - view.eventBorderWidth;
            } else {
